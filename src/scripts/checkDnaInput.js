...
const findMatches = (dna) => {
  ...
  const indexes = [];
  for (const match of matches) {indexes.push(match.index+1)}
  return indexes.join(', ')
};
...
export const checkDnaInput = (dna) => {
  ...
  for (let nt of dnaUpperArray) {
    if (nt.match(/[^ACGT]/)) 
    return(`Non-DNA character entered, please enter ATCG only.  Non-DNA characters at positions: ${findMatches(dna)}`);
    }
